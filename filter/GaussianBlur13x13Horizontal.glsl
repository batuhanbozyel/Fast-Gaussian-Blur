...
void main()
{
    ...
    color += texture2D(u_Texture, v_TexCoord + vec2(u_Offset[0] / u_Width, 0.0)) * u_Weight[1];
    color += texture2D(u_Texture, v_TexCoord - vec2(u_Offset[0] / u_Width, 0.0)) * u_Weight[1];

    color += texture2D(u_Texture, v_TexCoord + vec2(u_Offset[1] / u_Width, 0.0)) * u_Weight[2];
    color += texture2D(u_Texture, v_TexCoord - vec2(u_Offset[1] / u_Width, 0.0)) * u_Weight[2];

    color += texture2D(u_Texture, v_TexCoord + vec2(u_Offset[2] / u_Width, 0.0)) * u_Weight[3];
    color += texture2D(u_Texture, v_TexCoord - vec2(u_Offset[2] / u_Width, 0.0)) * u_Weight[3];
}